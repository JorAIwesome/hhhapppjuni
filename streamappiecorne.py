...
# Alle variabelen met standaardwaardes
MAX_CONSECUTIVE_DAYS = st.number_input("MAX_CONSECUTIVE_DAYS", value=5)
overwork_penalty_factor = st.number_input("overwork_penalty_factor", value=5)
spread_penalty_factor = st.number_input("spread_penalty_factor", value=40)
weekday_variance_penalty_factor = st.number_input("weekday_variance_penalty_factor", value=70)
shift_overload_penalty_factor = st.number_input("shift_overload_penalty_factor", value=1100)
consecutive_workdays_penalty_factor = st.number_input("consecutive_workdays_penalty_factor", value=30)
total_hours_penalty_factor = st.number_input("total_hours_penalty_factor", value=1)
blacklist_penalty_factor = st.number_input("blacklist_penalty_factor", value=75000)
free_weekday_penalty_factor = st.number_input("free_weekday_penalty_factor", value=50000)
free_nv_penalty_factor = st.number_input("free_nv_penalty_factor", value=4000)
scattered_shift_penalty_factor = st.number_input("scattered_shift_penalty_factor", value=3000)
branch_distribution_penalty_factor = st.number_input("branch_distribution_penalty_factor", value=2)
flex_worker_penalty_factor = st.number_input("flex_worker_penalty_factor", value=10)
underassignment_penalty_factor = st.number_input("underassignment_penalty_factor", value=9)
meeting_distance_penalty_factor = st.number_input("meeting_distance_penalty_factor", value=20)
weekend_cohesion_penalty_factor = st.number_input("weekend_cohesion_penalty_factor", value=500)
evv_day_shift_penalty_factor = st.number_input("evv_day_shift_penalty_factor", value=12)
non_evv_balance_penalty_factor = st.number_input("non_evv_balance_penalty_factor", value=10)
consecutive_night_penalty_factor = st.number_input("consecutive_night_penalty_factor", value=1000000)


if st.button("▶️ Run Script & Genereer Excel"):
    contract_naar_maand_factor = 6 # Om maandelijkse uren te berekenen
    max_hour_limit = 1.08 # Bepaalt hoeveel uren er overgewerkt mag worden op maandbasis
    ...
    #st.write(emp_file.columns)
    #st.write(emp_file[emp_file["naam"] == "Aafke de Boer"]["contract uren"])
    ...
    #st.write(prepare_prompt_for_translation(emp_file["harde wens"].iloc[10:20]))
    ...
    for batch_start in range(0, len(emp_file), batch_size):
        ...
        while attempt < max_retries:
            try:
                ...
                #st.write(response_json.get("vrije_dagen"))
                ...
                attempt += 1
                #st.write(f"Error occurred in batch {batch_start}-{batch_start+batch_size}: {e}. Attempt {attempt}/{max_retries}")
                time.sleep(2)
    ...
    for batch_start in range(0, len(emp_file), batch_size):
        ...
        while attempt < max_retries:
            try:
                ...
                #st.write(response_json["vakantiedagen"])
                # Ensure output length matches input length
    ...
    for batch_start in range(0, len(emp_file), batch_size):
        batch = emp_file.iloc[batch_start : batch_start + batch_size,]
        #st.write(batch[["vrije dagen (niet verlof)"]])
        ...
        while attempt < max_retries:
            try:
                ...
                #st.write(response_json["vrije_dagen_nv"])
                # Ensure output length matches input length
    ...
    for batch_start in range(0, len(emp_file), batch_size):
        batch = emp_file.iloc[batch_start : batch_start + batch_size,]
        #st.write(batch[["overleg"]].fillna("[]")) # add a dash to prevent truly empty inputs, which can confuse the model. 
    ...
    for batch_start in range(0, len(emp_file), batch_size):
        batch = emp_file.iloc[batch_start : batch_start + batch_size,]
        st.write(batch[["tijd wensen"]].fillna("[]"))
    ...
    for batch_start in range(0, len(emp_file), batch_size):
        batch = emp_file.iloc[batch_start : batch_start + batch_size,]
        st.write(batch[["scholing en tijd"]].fillna("[]")) # add a dash to prevent truly empty inputs, which can confuse the model. 
    ...
    st.write(emp_file["scholing_tijden"].iloc[40:80])
    st.write(emp_file["vrije_dagen"].iloc[20:50])
    st.write(emp_file["tijd_wensen"].iloc[70:90])
    ...
    st.write(get_days_in_month(REFERENCE_YEAR, REFERENCE_MONTH))
    ...
    st.write("📌 Branch ID Mapping:", branch_id_map)  # Debugging
    ...
    def parse_branch_distribution(dist_str):
        ...
        try:
            ...
            for pair in raw_pairs:
                ...
                st.write(branch_name, branch_id)
                weight = float(weight)        

                if weight < 1:
                    weight = weight * 100
                #st.write(branch_id, weight)
                branch_weights[branch_id] = weight
                st.write(branch_weights)
                total += weight
            ...
            #        st.write(branch_weights[key])
            st.write(branch_weights)
            return branch_weights

        except Exception as e:
            st.write(f"⚠️ Could not parse branch_distribution: {dist_str} — {e}")
            return {}
    ...
    def compute_net_hours(row):
        ...
        st.write(f'contracturen: {contract_hours}', f' netto uren: {net_hours}', f' aantal vakantiedagen: {len(weekday_vac_days)}', f' fte ratio: {vacation_ratio}', f' vacation hours: {vacation_hours}')
    ...
    class Shift:
        def __init__(self, shift_id, start_time, duration, allowed_functions=None, naam = "Geen naam", presence_mandatory =  False):
    ...
    SHIFT_PERIODS_PER_BRANCH = {
        ...
        "2": [ # West3
            ...
            {"name": "Middag3" , "start": 15.25, "end": 15.5 , "min_staff": 2},
    ...
    SHIFT_TYPES = {
        ...
        "0": [ # West1
            Shift(0, 7.0, 8.5,      allowed_functions=[5,6,7], naam = "Verantw dienst", presence_mandatory=True),
            Shift(1, 15.25, 8.25,   allowed_functions=[5,6,7], naam = "Verantw dienst", presence_mandatory=True),
            Shift(2, 7.5, 5.0,      allowed_functions=[0,1,2,3,4,5,6,7], naam = "Helpende dienst"),
            Shift(3, 8.0, 5.0,      allowed_functions=[0,1,2,3,4,8], naam = "Woonass dienst", presence_mandatory=True), # TODO: is functie 7 wel nodig op deze shifts
            Shift(4, 14.0, 8.5,     allowed_functions=[0,1,2,3,4,8], naam = "Woonass dienst", presence_mandatory=True),
            Shift(5, 23.25, 8.25,   allowed_functions=[0,1,2,3,4,5,6,7], naam = "Nacht dienst"),
        ],
        "1": [ # West2
            Shift(0, 7.0, 8.0,      allowed_functions=[5,6,7], naam = "Verantw dienst", presence_mandatory=True),
            Shift(1, 15.25, 8.25,   allowed_functions=[5,6,7], naam = "Verantw dienst", presence_mandatory=True),
            Shift(2, 7.0, 5.0,      allowed_functions=[0,1,2,3,4,5,6,7], naam = "Helpende dienst"),
            Shift(3, 8.5, 4.5,      allowed_functions=[0,1,2,3,4,8], naam = "Woonass dienst", presence_mandatory=True),
            Shift(4, 13.5, 8.5,     allowed_functions=[0,1,2,3,4,8], naam = "Woonass dienst"),
            Shift(5, 23.25, 8.25,   allowed_functions=[0,1,2,3,4,5,6,7], naam = "Nacht dienst"),     
        ], 
        "2": [ # West3
            Shift(0, 7.0, 8.0,      allowed_functions=[5,6,7], naam = "Verantw dienst", presence_mandatory=True),
            Shift(1, 15.25, 8.25,   allowed_functions=[5,6,7], naam = "Verantw dienst", presence_mandatory=True),
            Shift(2, 7.0, 3.0,      allowed_functions=[0,1,2,3,4,5,6,7], naam = "Helpende dienst"),
            Shift(3, 7.0, 5.0,      allowed_functions=[0,1,2,3,4,8], naam = "Woonass dienst", presence_mandatory=True),
            Shift(4, 7.5, 7.5,      allowed_functions=[0,1,2,3,4,8], naam = "Woonass dienst", presence_mandatory=True),
            Shift(5, 23.25, 8.25,   allowed_functions=[0,1,2,3,4,5,6,7], naam = "Nacht dienst"),
        ...
        "3": [ # Zuid
            Shift(0, 7.0, 8.5,      allowed_functions=[5,6,7], naam = "Verantw dienst", presence_mandatory=True),
            Shift(1, 15.25, 8.25,   allowed_functions=[5,6,7], naam = "Verantw dienst", presence_mandatory=True),
            Shift(2, 7, 6,          allowed_functions=[0,1,2,3,4,5,6,7], naam = "Helpende dienst"),
            Shift(3, 8.0, 5.5,      allowed_functions=[0,1,2,3,4,8], naam = "Woonass dienst", presence_mandatory=True),
            Shift(4, 14, 8,         allowed_functions=[0,1,2,3,4,8], naam = "Woonass dienst", presence_mandatory=True),
            Shift(5, 23.25, 8.25,   allowed_functions=[0,1,2,3,4,5,6,7], naam = "Nacht dienst"), # is deze wel nodig?
        ],
        "4": [ # Oost1
            Shift(0, 7.0, 8.5,      allowed_functions=[5,6,7], naam = "Verantw dienst", presence_mandatory=True),
            Shift(1, 15.25, 8.25,   allowed_functions=[5,6,7], naam = "Verantw dienst", presence_mandatory=True),
            Shift(2, 7, 5,          allowed_functions=[0,1,2,3,4,5,6,7], naam = "Helpende dienst"),
            Shift(3, 8.25, 4.5,     allowed_functions=[0,1,2,3,4,8], naam = "Woonass dienst", presence_mandatory=True),
            Shift(4, 13.5, 8.5,     allowed_functions=[0,1,2,3,4,8], naam = "Woonass dienst", presence_mandatory=True),
            Shift(5, 23.25, 8.25,   allowed_functions=[0,1,2,3,4,5,6,7], naam = "Nacht dienst"),
        ],
        "5": [ # Oost2
            Shift(0, 7.0, 8.5,      allowed_functions=[5,6,7], naam = "Verantw dienst", presence_mandatory=True),
            ...
            Shift(2, 7.5, 5.0,      allowed_functions=[0,1,2,3,4,5,6,7], naam = "Helpende dienst"),
            Shift(3, 8.5, 4.5,      allowed_functions=[0,1,2,3,4,8], naam = "Woonass dienst", presence_mandatory=True),
            Shift(4, 13.75, 8.5,    allowed_functions=[0,1,2,3,4,8], naam = "Woonass dienst", presence_mandatory=True),
            Shift(5, 23.25, 8.25,   allowed_functions=[0,1,2,3,4,5,6,7], naam = "Nacht dienst"),
        ],
        "6": [ # Oost3
            Shift(0, 7.0, 8.5,      allowed_functions=[5,6,7], naam = "Verantw dienst", presence_mandatory=True),
            Shift(1, 7.0, 8.5,      allowed_functions=[0,1,2,3,4,5,6,7], naam = "Helpende dienst"), # moet presence
            Shift(2, 7.0, 5.0,      allowed_functions=[0,1,2,3,4,5,6,7], naam = "Helpende plus dienst"),
            Shift(3, 7.0, 3.0,      allowed_functions=[0,1,2,3,4,5,6,7], naam = "Helpende dienst"),
            Shift(4, 7.5, 5.5,      allowed_functions=[0,1,2,3,4,8], naam = "Woonass dienst", presence_mandatory=True),
            Shift(5, 15.25, 8.25,   allowed_functions=[5,6,7], naam = "Verantw dienst", presence_mandatory=True),
            Shift(6, 17.0, 5.0,     allowed_functions=[0,1,2,3,4,5,6,7], naam = "Helpende dienst"),
            Shift(7, 16.0, 4.0,     allowed_functions=[0,1,2,3,4,8], naam = "Woonass dienst", presence_mandatory=True),
            Shift(8, 23.25, 8.25,   allowed_functions=[0,1,2,3,4,5,6,7], naam = "Nacht dienst"),
        ],
        "7": [ # Aanleun
            Shift(0, 7.0, 8.5,      allowed_functions=[5,6,7], naam = "Verantw dienst", presence_mandatory=True),
            Shift(1, 15.25, 8.25,   allowed_functions=[5,6,7], naam = "Verantw dienst", presence_mandatory=True),
            Shift(2, 7.5, 5.0,      allowed_functions=[0,1,2,3,4,5,6,7], naam = "Helpende dienst"),
            Shift(3, 23.25, 8.25,   allowed_functions=[0,1,2,3,4,5,6,7], naam = "Nacht dienst"),
        ],
    }
    ...
    FUNCTION_COVERAGE_REQUIREMENTS = [
        ...
        #     "function_ids": [0,1,2,3,4,5,6,7],
        #     "qualified_ids": [5,6],               # Functies die zelfstadig een verantwoorde dienst mogen draaien
        #     "min_staff": 2,
        #     "time_window": (1.0, 2.0),           # Night shift coverage
        # },
    ]
    ...
    st.write(BUILDING_TO_BRANCHES.items())
    st.write(TIME_SLOT_TO_MIN_STAFF.get(str(6), {}).get(round(1, 1), 0))
        ...
        def on_solution_callback(self):
            ...
            for (e, d, s, b), var in self.works.items():
                if self.Value(var):
                    ...
                    st.write(f"  - {employee.name} (func: {employee.func}) → Day {d + 1}, Branch {b}, Shift: {shift.start_time:.2f}–{shift_end:.2f}")
    ...
    class OrToolsScheduler:
        ...
        def _add_constraints(self):
                        ...
                        for e in range(self.num_employees):
                            ...
                            for s, shift in enumerate(SHIFT_TYPES.get(str(branch), [])):
                                ...
                                if shift_start <= t < shift_end and (e, d, s, branch) in self.works:
                                    ...
                                    #st.write("day ASSIGNED")
                        if staff_covering_slot:
                            #st.write(f"Medewerker: ", {e}, " dag: ", {d + 1}, " shift: ", {s}, " branch: ", {branch})
            ...
            #        #st.write(e, employee, d, day_of_week, unavailable_days)
                            ...
                            for b in employee.allowed_branches:
                                for s, shift in enumerate(SHIFT_TYPES.get(str(b), [])):
                                    ...
                                    if overlap and (e, day, s, b) in self.works:
                                        #st.write(f"Medewerker: ", {e}, " dag: ", {day + 1}, " shift: ", {s}, " branch: ", {b})
                        ...
                        for s, shift in enumerate(SHIFT_TYPES.get(str(b), [])):
                            ...
                            for time_range in preferred_time_by_weekday[weekday]:
                                ...
                                if shifts_in_preferred_time(shift_start, shift_end, pref_start, pref_end):
                                    ...
                                    #     st.write(shift_start, shift_end, pref_start, pref_end)
                                    allowed = True
            ...
            if shift.presence_mandatory and eligible_employees:
                ...
                st.write(f"⚠️ No eligible employee found for mandatory shift on day {d}, branch {b}, shift {s}")
                ...
                for d in range(self.num_days):
                    ...
                    for b in covered_branches:
                        for s, shift in enumerate(SHIFT_TYPES.get(str(b), [])):
                            ...
                            covers_time = (
                                (shift_start < end_t and shift_end > start_t)
                                if start_t < end_t
                                else (shift_start >= start_t or shift_end <= end_t)
                            )
                            ...
                            if covers_time:
                                for e in range(self.num_employees):
                                    ...
                                    if (
                                        str(b) in emp.allowed_branches
                                        and emp.func in func_ids
                                        and (e, d, s, str(b)) in self.works
                                    ):
                                        total_staff.append(self.works[(e, d, s, str(b))])

                                        if emp.func in qualified_ids:
                                            qualified_staff.append(self.works[(e, d, s, str(b))])
            ...
            #     estimated_max_weekly_shifts = math.ceil(employee.contract_hours / 4.3 / 8) + 1 # e.g. 20 hours -> 3 + 1 = 4 days per 7 days at most
        ...
        def solve(self):
            ...
            for e, employee in enumerate(self.employees):
                estimated_max_shifts = math.ceil((employee.contract_hours or 0) / 4.3 / 8)
                    ...
                    for b in self.employees[e].allowed_branches:
                        for s, shift in enumerate(SHIFT_TYPES.get(str(b), [])):
                            if (e, d, s, b) in self.works:
                                ...
                                term = cp_model.LinearExpr.Term(self.works[(e, d, s, b)], duration)
                ...
                for d in range(self.num_days):
                    for b in employee.allowed_branches:
                        for s, shift in enumerate(SHIFT_TYPES.get(str(b), [])):
                            ...
                            if key in self.works:
                                duration = int(shift.duration)
                                expr = cp_model.LinearExpr.Term(self.works[key], duration)
                ...
                for meeting in employee.meeting_times:  # Each element is a dict like {"12": "14-16"}
                    for day_str, time_range in meeting.items():
                        try:
                            ...
                            st.write(f"⚠️ Error parsing meeting time for employee {e} on day {day_str}: {ex}")
            ...
            for time_limit in time_values:
                ...
                for seed in seeds:
                    ...
                    if result not in (cp_model.OPTIMAL, cp_model.FEASIBLE):
                        st.write(f"❌ No feasible solution (status: {solver.StatusName(result)})")
                        return None, None
                    ...
                    for name, soft_constraint_penalty in penalties_to_track.items():
                        ...
                        st.write(f"Actual violations found for {name}:", violations)
                        st.write("Reported objective penalty score:", solver.ObjectiveValue())
                    
                

                    if result in (cp_model.OPTIMAL, cp_model.FEASIBLE):
                        score = solver.ObjectiveValue()
                        st.write(f"✅ Seed {seed} | Time {time_limit}s → Score: {score:.2f}")
                        results.append({
                            "seed": seed,
                        ...
                        st.write(f"❌ Seed {seed} | Time {time_limit}s → No solution")
                        results.append({
                            "seed": seed,
                            "time_limit": time_limit,
                            "score": None
            ...
            #if result not in (cp_model.OPTIMAL, cp_model.FEASIBLE):
            #    st.write("Geen roosteroplossing gevonden.")
            #    return []
                    ...
                    for b in self.employees[e].allowed_branches:
                        for s, shift in enumerate(SHIFT_TYPES.get(str(b), [])):  # ✅ Use branch-specific shifts
                            if best_schedule.Value(self.works[(e, d, s, b)]) == 1:
                                ...
                                employee.assigned_hours += shift.duration
    ...
    # Resultaat tonen als tabel
    st.write(f"{'Datum':15} {'Naam':15} {'Afdeling':10} {'Starttijd':7} {'Duur':7} {'Shift':20} {'Functie' :15}" )
    #st.write(rooster[:10])
    for entry in rooster[:10]:  # toon bv. eerste 10 geplande diensten
        st.write(entry)

    st.write(pd.DataFrame(rooster))
    ...
    # Belangrijk om geen error te krijgen als folder nog niet is aangemaakt.

    os.mkdir('Output_Roosters')

    df_rooster_pivot.to_excel(f"Output_Roosters/Rooster_output_HHH_{REFERENCE_YEAR}_{REFERENCE_MONTH:02d}.xlsx", index = False)
    ### --------------------------------------------------------------- #### --------------------------------------------------------------- #### ---------------------------------------------------------------
